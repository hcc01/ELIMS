...
bool Worksheet::mergeCells(const CellRange &range, const Format &format)
{
    ...
    for (int row = range.firstRow(); row <= range.lastRow(); ++row) {
        for (int col = range.firstColumn(); col <= range.lastColumn(); ++col) {
            ...
            d->cellMerge[QPair(row,col)]=range;//修改内容：添加单元格的合并情况
        }
    }
    qDebug()<<"new merge:"<<range.toString();
...
bool Worksheet::unmergeCells(const CellRange &range)
{
    Q_D(Worksheet);
    //修改内容：删除单元格的合并情况
    for (auto it = d->cellMerge.begin(); it != d->cellMerge.end();) {
        if (it.value() == range) {
            // 删除当前迭代器指向的项
            it = d->cellMerge.erase(it);
        } else {
            // 继续到下一个项
            ++it;
        }
...
bool Worksheet::isMergeCell(int row, int column) const
{
    Q_D(const Worksheet);
    return d->cellMerge.contains(QPair(row,column));
}

bool Worksheet::isFirstMergeCell(int row, int column) const
{
    Q_D(const Worksheet);
    CellRange range=d->cellMerge.value(QPair(row,column));
    if(range.firstRow()==row&&range.firstColumn()==column) return true;
    return false;
}

CellRange Worksheet::mergeRange(int row, int column) const
{
    Q_D(const Worksheet);
    if(isMergeCell(row,column)){
        return d->cellMerge.value(QPair(row,column));
    }
    return CellRange();
}
...
//                merges.append(CellRange(rangeStr));
                //为了保存单元格的合并情况
                CellRange rg(rangeStr);
                merges.append(rg);
                for(int r=rg.firstRow();r<=rg.lastRow();r++){
                    for(int c=rg.firstColumn();c<=rg.lastColumn();c++){
                        cellMerge[QPair(r,c)]=rg;
                    }
                }
            }
        }
